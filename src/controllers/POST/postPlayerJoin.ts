...
export default {
    ...
    handler: async (req: FastifyRequest, reply: FastifyReply, database: database) => {
        ...
        const dbResults = await database.promisedQuery("SELECT * FROM users WHERE uuid = ? AND mc_server = ?", [uuid, mc_server]).catch(err => console.log(err));
        ...
        if (!dbResults.length) {
            try {
                await database.promisedQuery("INSERT INTO users(username, joindate, uuid, joins, mc_server) VALUES (?,?,?,?,?)", [user, time, uuid, 1, mc_server]);
        ...
        if (dbResults.length > 0 && (uuid === dbResults[0].uuid && user !== dbResults[0].username)) {
            try {
                await database.promisedQuery("UPDATE users SET username = ? WHERE username = ? AND uuid = ? AND mc_server = ?", [user, dbResults[0].username, uuid, mc_server]);
        ...
        if (dbResults.length > 0 && (uuid === dbResults[0].uuid && user === dbResults[0].username)) {
            try {
                await database.promisedQuery("UPDATE users SET joins = joins + 1, lastseen = ? WHERE username = ? AND mc_server = ?", [time, user, mc_server]);
